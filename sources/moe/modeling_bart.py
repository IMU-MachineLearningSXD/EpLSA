...
class BartMoEForConditionalGeneration(PretrainedBartModel):
    ...
    def generate(
        ...
        if self.config.is_encoder_decoder:
            ...
            sent_encoder = self.get_sentEncoder()
            ...
            if self.config.mixture_embedding:
                ...
                # sentence_representation = encoder_outputs[0][eos_mask, :].view(encoder_outputs[0].size(0), -1,
                #                                                                encoder_outputs[0].size(-1))[:, -1, :]
                # sent_encoder_outputs = sent_encoder(encoder_outputs[0][:,0,:], return_dict=True,is_pos=True)
                # encoder_outputs['last_hidden_state'] = sent_encoder_outputs['last_hidden_state'] + encoder_outputs[
                #     'last_hidden_state']
