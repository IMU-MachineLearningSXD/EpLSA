...
class Seq2SeqTrainer(Trainer):
    def __init__(self, config, data_args, *args, **kwargs):
        ...
        self.dropout = nn.Dropout(p=config.classif_dropout)
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            if epoch < int(np.ceil(num_train_epochs)) and self.args.evaluate_during_training:
                ...
                if metrics["self_bleu_4"] < self.rouge:
                    self.rouge = metrics["self_bleu_4"]
                    self._save_training2(model, trial, metrics=metrics)
            if self.args.max_steps > 0 and self.global_step >= self.args.max_steps:
                break
    ...
    def _save_training2(self, model, trial, metrics=None):
        # In all cases (even distributed/parallel), self.model is always a reference
        # to the model we want to save.
        if hasattr(model, "module"):
            assert model.module is self.model, f"Module {model.module} should be a reference to self.model"
            ...
            assert model is self.model, f"Model {model} should be a reference to self.model"
        # Save model checkpoint
        checkpoint_folder = f"checkpoint_best_dev_rouge_l"
        if self.hp_search_backend is not None and trial is not None:
            run_id = trial.number if self.hp_search_backend == HPSearchBackend.OPTUNA else tune.get_trial_id()
            checkpoint_folder += f"-run-{run_id}"
        output_dir = os.path.join(self.args.output_dir, checkpoint_folder)
            ...
            if self.args.local_rank != -1:
                metrics["eval_loss"] = (
                    distributed_broadcast_scalars(eval_losses,
                                                  num_total_examples=self.num_examples(dataloader)).mean().item())
            else:
